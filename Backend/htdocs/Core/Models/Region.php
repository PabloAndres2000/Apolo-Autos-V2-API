...
class Region
{
    ...
    public function Insertar()
    {
        ...
        if ($query->execute()) {
            $response = new Response(200, "Region agregada correctamente");
        } else {
    ...
    public function Listar()
    {
        ...
        if ($query->num_rows > 0) {
            ...
            $response = new Response(404, "No se han encontrado registros de las regiones");
        }
        //se cierra la conexion a la base de datoss
        $db->con->close();
        //se cierra la consulta que se hace mediante la variable $query
    ...
    public function Actualizar()
    {
        ...
        $query = $db->con->prepare("UPDATE regiones SET nombre = ? WHERE idregion = ?");
        $query->bind_param("si", $this->nombre, $this->idregion);
        if ($query->execute()) {
